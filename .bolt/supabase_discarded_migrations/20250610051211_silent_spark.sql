/*
  ...
  1. New Tables
    - `stripe_config` - Configuración de Stripe (API keys, webhooks)
    - `stripe_products` - Productos sincronizados con Stripe
    - `stripe_prices` - Precios de productos de Stripe
    - `stripe_transactions` - Registro de transacciones
    - `stripe_webhooks` - Log de webhooks recibidos

  2. Security
    - Enable RLS on all tables
    - Only super admin can access configuration
