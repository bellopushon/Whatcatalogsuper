...
export default function PlanManagement() {
  const { state, createPlan, updatePlan, deletePlan, loadPlans } = useSuperAdmin();
  ...
  const handleCreatePlan = async () => {
    ...
    try {
      ...
      setIsCreating(false);
      resetForm();
      success('Plan creado', 'El plan se ha creado correctamente y se sincronizó con el proyecto principal');
    } catch (err: any) {
  ...
  const handleUpdatePlan = async () => {
    ...
    try {
      ...
      setEditingPlan(null);
      resetForm();
      success('Plan actualizado', 'El plan se ha actualizado correctamente');
    } catch (err: any) {
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            ...
            <p className="text-slate-400 text-sm lg:text-base mt-1">
              Crea y administra los planes de suscripción. La lógica se basa en las características del plan.
            </p>
          </div>
      ...
      <div className="p-4 lg:p-8 space-y-6">
        ...
        <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="text-blue-400 font-medium text-sm">Sistema de Planes Dinámico</h3>
              <p className="text-blue-300/80 text-sm mt-1">
                Los planes se basan en características y niveles. Puedes crear, editar y eliminar planes. 
                La lógica del sistema se adapta automáticamente a los planes disponibles.
              </p>
            </div>
          </div>
        </div>
            ...
            <div key={plan.id} className={`rounded-lg border-2 p-4 lg:p-6 ${getPlanTypeColor(plan)}`}>
              ...
              <div className="mb-4">
                <div className="flex items-baseline gap-1">
                  ...
                  {plan.price > 0 && <span className="text-slate-400">/mes</span>}
                </div>
                <div className="text-xs text-slate-500 mt-1">Nivel {plan.level}</div>
              </div>
