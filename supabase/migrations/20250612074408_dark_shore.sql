/*
  ...
  This migration converts the users.plan column from text to uuid to properly reference the plans table.
  
  1. Drop dependent objects (triggers, views, functions)
  2. Migrate data from text/enum to UUID references
  3. Recreate all dependent objects with proper UUID handling
...
-- Step 4: Create a temporary column to store UUID values
...
-- Step 10: Create function to get default plan ID (after dropping the old one)
...
-- Step 15: Set the default value using the function
