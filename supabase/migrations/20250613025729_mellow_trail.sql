/*
  # Fix Users Plan Column Migration - Convert from text/enum to UUID

  This migration converts the users.plan column from text/enum to UUID foreign key reference.
  
  1. Database Changes
    - Drop existing triggers and view that depend on the plan column
    - Add temporary UUID column for migration
    - Migrate existing data with proper type casting
    - Replace old column with new UUID column
    - Add foreign key constraint to plans table
    - Recreate triggers and view with UUID references
    
  2. Data Migration
    - Handle plan names by matching with plans.name
    - Handle UUID strings by casting to UUID type
    - Ensure all users have valid plan assignments
    - Create default free plan if needed
    
  3. Security
    - Maintain all existing RLS policies
    - Recreate all validation triggers
...
-- Step 4: Migrate existing data with proper type casting
-- First, handle cases where plan column contains plan names (text comparison)
UPDATE public.users u
SET plan_uuid = p.id
FROM public.plans p
WHERE u.plan::text = p.name::text
  AND u.plan_uuid IS NULL;

-- Handle cases where plan column contains UUID strings
-- Use a safe approach with exception handling
DO $$
DECLARE
    user_record RECORD;
    plan_uuid_val uuid;
BEGIN
    ...
    LOOP
        BEGIN
            -- Try to cast the plan value to UUID
            plan_uuid_val := user_record.plan::uuid;
            
            -- Check if this UUID exists in plans table
            IF EXISTS (SELECT 1 FROM public.plans WHERE id = plan_uuid_val) THEN
                UPDATE public.users 
                SET plan_uuid = plan_uuid_val 
...
DO $$
DECLARE
    users_without_valid_plan integer;
BEGIN
    SELECT COUNT(*) INTO users_without_valid_plan
    FROM public.users
    WHERE plan_uuid IS NULL;
    
    IF users_without_valid_plan > 0 THEN
        RAISE EXCEPTION 'Migration failed: % users still without valid plans', users_without_valid_plan;
    END IF;
END $$;
...
-- Step 10: Set default value using the first free plan
DO $$
DECLARE
...
-- Step 11: Recreate the user_plan_details view
...
-- Step 12: Recreate the plan validation trigger function and trigger
...
-- Step 13: Recreate the plan upgrade trigger function and trigger
...
-- Step 14: Recreate the plan downgrade trigger function and trigger
...
-- Step 15: Update any existing user preferences that might reference invalid stores
...
-- Step 16: Clean up the helper function (no longer needed)
