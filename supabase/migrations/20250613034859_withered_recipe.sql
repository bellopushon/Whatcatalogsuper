/*
  # Fix Users Plan Column Migration

  This migration converts the users.plan column from text/enum to UUID foreign key reference.
  
  1. Database Changes
     - Convert users.plan from text to UUID
     - Add foreign key constraint to plans table
     - Migrate existing data safely
     - Set default plan for new users
  
  2. Triggers and Functions
     - Recreate plan validation triggers
     - Recreate plan upgrade/downgrade handlers
     - Update user_plan_details view
  
  3. Data Migration
     - Handle both plan names and UUID strings
     - Ensure all users have valid plans
...
-- First, handle cases where plan column contains plan names (cast plan to text explicitly)
UPDATE public.users u
SET plan_uuid = p.id
FROM public.plans p
WHERE u.plan::text = p.name
...
-- Step 5: Handle any remaining enum values by mapping them to plan names
UPDATE public.users u
SET plan_uuid = p.id
FROM public.plans p
WHERE u.plan_uuid IS NULL
  AND u.plan IS NOT NULL
  AND (
    (u.plan::text = 'gratuito' AND p.name ILIKE '%gratuito%') OR
    (u.plan::text = 'emprendedor' AND p.name ILIKE '%emprendedor%') OR
    (u.plan::text = 'profesional' AND p.name ILIKE '%profesional%')
  )
  AND p.is_active = true;

-- Step 6: Ensure all users have valid plan assignments
DO $$
DECLARE
    free_plan_id uuid;
...
-- Step 10: Set default value using a function approach
-- First create a function to get the free plan ID
...
-- Step 15: Now set the default value using the function
-- We do this after creating triggers to avoid any conflicts
DO $$
DECLARE
...
-- Step 16: Update any existing user preferences that might reference invalid stores
